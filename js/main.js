...
function initializeCrossQuoteWidget() {
  if (!Array.isArray(CROSS_QUOTES) || CROSS_QUOTES.length === 0) return;
  ...
  if (!Number.isFinite(quoteSeed) || quoteSeed < 0 || quoteSeed >= CROSS_QUOTES.length) {
    quoteSeed = Math.floor(Math.random() * CROSS_QUOTES.length);
    window.localStorage.setItem(`${quoteKeyPrefix}-seed`, String(quoteSeed));
  }

  const quoteIndex = (Math.floor(daysSinceEpoch / rotationPeriodDays) + quoteSeed) % CROSS_QUOTES.length;
  const quoteText = CROSS_QUOTES[quoteIndex];
